...
export default function DogSVG({ breed = {}, animation = null, size = 120, pose = 'side' }) {
  ...
  return (
    <svg viewBox="0 0 160 160" width={size} height={size} className={cls} style={animVars} aria-hidden="true">
      ...
      {sleeping && (
        <>
          <text className="zzz-letter"     x="148" y="55" fill={accentColor} style={{fontSize:'16px', fontWeight:'bold', fontFamily:'serif'}}>z</text>
          <text className="zzz-letter zzz-d1" x="140" y="38" fill={accentColor} style={{fontSize:'13px', fontWeight:'bold', fontFamily:'serif'}}>z</text>
          <text className="zzz-letter zzz-d2" x="133" y="24" fill={accentColor} style={{fontSize:'10px', fontWeight:'bold', fontFamily:'serif'}}>z</text>
        </>
      )}

      {/* ── 꼬리 — 몸통 연결점(36,100) 기준 회전 ── */}
...
function TailShape({ type, bodyColor, accentColor }) {
  if (type === 'curly')  return <path d="M 36 100 Q 20 80 28 65 Q 36 50 45 62 Q 52 72 42 78" stroke={accentColor} strokeWidth="8" fill="none" strokeLinecap="round" />
  if (type === 'short')  return <ellipse cx="30" cy="96" rx="9" ry="12" fill={accentColor} transform="rotate(20 30 96)" />
  if (type === 'long')   return <path d="M 36 100 Q 8 80 14 55 Q 18 40 30 50" stroke={bodyColor} strokeWidth="9" fill="none" strokeLinecap="round" />
  if (type === 'pom')    return (
    <>
...
function EarShape({ type, side, accentColor }) {
  if (type === 'pointy') {
  ...
  if (type === 'curly') {
    if (side === 'left') return <ellipse cx="108" cy="62" rx="13" ry="18" fill={accentColor} />
    return <ellipse cx="130" cy="55" rx="13" ry="17" fill={accentColor} opacity="0.7" />
  }
  // floppy
  if (side === 'left') return <ellipse cx="108" cy="70" rx="11" ry="18" fill={accentColor} transform="rotate(-15 108 56)" />
  return <ellipse cx="130" cy="60" rx="10" ry="16" fill={accentColor} opacity="0.7" transform="rotate(10 130 48)" />
}
